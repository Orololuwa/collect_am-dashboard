...
const AddReceiptBatch = (): JSX.Element => {
  ...
  const uploadBatchHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      addToast("Receipts added successfully", {
        appearance: "success"
      });
      setLoading(false);
      navigate("../receipts");
  ...
  return (
    <>
      <Wrapper className="container">
        <h5 className="text-4xlxl">Add Receipts</h5>
        <form className="my-10" onSubmit={uploadBatchHandler}>
          <div className="py-2">
            <label htmlFor="receiptsBatch">Upload Excel Files*</label>
            <InputBlock w="30rem" type="file" id="receiptsBatch" />
          </div>
...
export default AddReceiptBatch;
